...
is non-negative. By \prettyref{lem:boundOnDegreesOfFA} the column
degrees of $\mathbf{F}\mathbf{P}_{2}$ are bounded by the $\vec{s}$-column
degrees $\vec{b}'$ of $\mathbf{P}_{2}$ . By \prettyref{lem:boundOfSumOfShiftedDegreesOfNullspaceBasis},
...
Now \prettyref{lem:multiplyUnbalancedMatrices} applies.
\end{pf}
...
At this point, the problem is reduced to computing a $\vec{t}$-minimal
...
algorithm, shown in \prettyref{alg:minimalNullspaceBasis}. \include{algorithmNullspaceBasis}
...
and \prettyref{line:multiplyFP2} cost $O^{\sim}\left(nm^{\omega-2}\xi\right)=O^{\sim}\left(m^{\omega-1}\xi\right)$.
The remaining operations including multiplications on \prettyref{line:multiplyG2N1}
and \prettyref{line:multiplyP2N1N2} costs $O^{\sim}\left(m^{\omega-1}\xi\right)$.
...
at \prettyref{line:multiplyFP2} cost $O^{\sim}\left(nm^{\omega-1}s\right)$.
\begin{lem}
...
at \prettyref{line:multiplyFP2} cost \textup{$O^{\sim}\left(nm^{\omega-1}s\right)$.}\end{lem}
\begin{pf}
...
$m$. For multiplying $\mathbf{F}$ and $\mathbf{P}_{2}$, \prettyref{lem:multiplyUnbalancedMatrices}
