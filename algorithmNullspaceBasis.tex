\begin{algorithm}[t]
\caption{$\mnb(\mathbf{F},\vec{s})$ Compute $\vec{s}$-Minimal Nullspace Basis}
...
\ifbody{\RETURN $\mathbf{P}_{1},\deg_{\vec{s}}\mathbf{P}_{1}$ 
...
\STATE{$\left[\mathbf{N}_{2},\vec{v}\right]:=\mnb\left(\mathbf{G}_{2}\mathbf{N}_{1},\vec{u}\right);$}\label{line:multiplyG2N1}

\STATE{$\mathbf{Q}:=\mathbf{N}_{1}\mathbf{N}_{2}$;}\label{line:multiplyN1N2}

\RETURN $\left[\mathbf{P}_{1},\mathbf{P}_{2}\mathbf{Q}\right],\left[\deg_{\vec{s}}\mathbf{P}_{1},\vec{v}\right]$\label{line:multiplyP2Q}}
\end{algorithmic}
\end{algorithm}
